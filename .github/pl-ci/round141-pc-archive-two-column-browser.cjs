...
(async()=>{let browser;try{
 ...
 for(const width of [320,390,1440]){
  ...
  const ruleProbe=await page.evaluate(()=>({coc7:pcRuleIsCoc(testPc(2)),coc6:pcRuleIsCoc({ruleMeta:{familyId:'brp',systemId:'coc',editionId:'6e'}}),brp:pcRuleIsCoc({ruleMeta:{familyId:'brp',systemId:'brp-generic',editionId:''}})}));
  assert.deepEqual(ruleProbe,{coc7:true,coc6:false,brp:false},`${width}: CoC7-only archive gate drift`);
